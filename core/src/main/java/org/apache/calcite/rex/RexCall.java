 ...
 *
 * 一种表达式，由以0个或多个表达式作为操作数的运算符的调用形成
 * 运算符可以是二进制的、一元的、函数的、特殊的句法结构，如CASE...WHEN...END，
 * 甚至内部生成的结构，如隐式类型转换。运算符的语法实际上是无关的，因为行表达式(与SQL表达式不同)并不直接表示源代码。
 *
 * 通常不需要子类化这个类。智能应该在操作符中，而不是在调用中。关于调用的任何额外信息通常都可以编码为额外的参数。(这些不需要隐藏，因为没有人会从这个树生成源代码。)
 *
 */
